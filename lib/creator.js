...
function checkValidConfig(config) {
  if (!JSON.parse(config).dockrr) throw new Error('Cannot Find "dockrr" config in package.json');
  if (!JSON.parse(config).dockrr.cmd) throw new Error('"dockrr" config requires "cmd" object');
  if (!JSON.parse(config).dockrr.expose) throw new Error('"dockrr" config requires "expose" object');
  if (JSON.parse(config).dockrr.label) {
    const labelContent = JSON.parse(config).dockrr.label;
    if (Array.isArray(labelContent) === false) throw new Error('"dockrr" config requires "label" to be an array');
  }
  if (JSON.parse(config).dockrr.env) {
    const envContent = JSON.parse(config).dockrr.env;
    if (Array.isArray(envContent) === false) throw new Error('"dockrr" config requires "env" to be an array');
  }
}
...
async function generateDockerFile() {
  try {
    ...
    const config = JSON.parse(configFile).dockrr;
...
async function appPort() {
  const configFile = await readConfig();
  checkValidConfig(configFile);
  return JSON.parse(configFile).dockrr.expose;
}
